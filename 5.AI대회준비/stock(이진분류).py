...
model = models.Sequential([#models.Sequential: Keras에서 제공하는 신경망 모델의 기본 클래스입니다. 레이어를 순차적으로 쌓아서 모델을 구성합니다. 각 레이어는 이전 레이어의 출력을 입력으로 받습니다.
    ...
    #L2 정규화(L2 regularization)를 적용하여 모델의 가중치가 너무 커지는 것을 방지합니다. 
    # 정규화는 과적합을 줄이고 모델의 일반화 능력을 향상시킵니다
    layers.Dense(32, activation='relu', input_shape=(X_train.shape[1],), kernel_regularizer=regularizers.l2(0.001)),
    #layers.Dropout(0.3): Dropout 레이어입니다. 
    # 학습 중에 30%의 뉴런을 무작위로 드롭(비활성화)하여 네트워크의 과적합을 방지합니다. 
    # 이를 통해 모델이 훈련 데이터에 지나치게 의존하지 않고 일반화 능력을 개선합니다.
    layers.Dropout(0.3),
    layers.Dense(16, activation='relu', kernel_regularizer=regularizers.l2(0.001)),
    layers.Dropout(0.3),
    layers.Dense(8, activation='relu'),
    #activation='sigmoid': 시그모이드 활성화 함수입니다. 출력값을 0과 1 사이로 압축합니다. 
    # 이진 분류 문제에서 각 샘플이 특정 클래스에 속할 확률을 나타냅니다.
...
# Early Stopping 콜백 설정: validation loss가 10번 동안 개선되지 않으면 학습 중지
